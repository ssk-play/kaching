...
- Order data is not retained beyond a rolling list of recently seen order IDs,
  kept locally so the same order is not announced twice.

## Where data is stored

In `chrome.storage.local` on your own machine. Uninstalling the extension
removes it. **Reset history** in the settings clears the record of seen orders.
