...
const system = (today) =>
  [
    ...
    'by the currency the buyer paid in; read_by_kind splits it by whether each sale was a',
    'one-off purchase, a new subscription or a renewal. Both splits take any range, so use',
    'read_by_currency for a question about a currency or a country, read_by_kind for one about',
    'subscriptions or renewals, and either for a total spanning more than two months.',
    ...
    'against them. If a day is flagged uncounted, say that its figure is short, and if a read',
    'comes back with daysNotSplit, say that much of the range was recorded before that split',
    'existed and that /recount fills those days in.',
